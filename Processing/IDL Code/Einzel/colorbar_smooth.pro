...
PRO COLORBAR_SMOOTH, BOTTOM=bottom, up=up, CHARSIZE=charsize, COLOR=color, DIVISIONS=divisions, $
...
;Had to add this in for nice colors to PS - A.P. 10/22/10
IF !d.name EQ 'PS' then device, decompose=1,/color,bits_per_pixel = 8
    ...
    IF N_ELEMENTS(up) EQ 0 THEN up = 255B
...
;    ENDIF
    ...
    ; Scale the color bar.
     bar = BYTSCL(bar, TOP=(ncolors-1) < (up-bottom)) + bottom
...
if !d.name EQ 'PS' then device, decompose=0
